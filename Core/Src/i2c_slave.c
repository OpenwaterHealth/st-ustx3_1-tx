...
#define I2C_GLOBAL_DEVICE hi2c2
...
void I2C_Slave_Init(uint8_t addr) {
  ...
  if(addr == 0x00 || addr > 0x7F){
	  I2C_GLOBAL_DEVICE.Init.OwnAddress1  = 0x32 << 1;  // default to 32
  }else{
	  I2C_GLOBAL_DEVICE.Init.OwnAddress1  = addr << 1;
  }

  data_available = NULL;
  // Reinitialize the I2C peripheral with the updated configuration
  if (HAL_I2C_Init(&I2C_GLOBAL_DEVICE) != HAL_OK) {
  ...
  if(HAL_I2C_EnableListen_IT(&I2C_GLOBAL_DEVICE) != HAL_OK) {
	  // Handle the error if reinitialization fails
	  Error_Handler();
  }
...
void i2c_print_info() {
    //uint32_t timing = I2C_GLOBAL_DEVICE.Init.Timing;
    ...
    printf("I2C Slave Addr: 0x%02x\r\n\r\n", (uint8_t)(I2C_GLOBAL_DEVICE.Init.OwnAddress1 >> 1));
}
...
{
	if(I2cHandle->Instance == I2C_GLOBAL_DEVICE.Instance) {
